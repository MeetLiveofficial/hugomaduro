...
class ProfileScreenController extends BlockUserController with GetTickerProviderStateMixin {
  ...
  void onAddPost({Post? post, CreateFeedType? type}) {
    ...
    if (type == CreateFeedType.feed) {
      posts.insert(pinnedCount, post);
    } else {
      reels.insert(pinnedCount, post);
    }
  }
    ...
    if (post.isPinned == 0) {
      ...
      if ((settingData?.maxPostPins ?? AppRes.maxPinFeed) > existingPinPost.length) {
        ...
        showSnackBar(LKey.pinLimitExceeded.trParams(
            {'pin_count': '${settingData?.maxPostPins ?? AppRes.maxPinFeed.toString()}'}));
      }
    } else {
    ...
    if ((settingData?.maxPostPins ?? AppRes.maxPinFeed) > existingPinPost.length) {
      ...
      if (response.status == true) {
        ...
        if (Get.isRegistered<PostScreenController>(tag: '${post.id}')) {
          Get.find<PostScreenController>(tag: '${post.id}').updatePost(post);
        }
        posts.insert(0, post);
        posts.refresh();
      }
  ...
  void openFollowList(FollowListType type) {
    final user = userData.value;
    final id = user?.id?.toInt();
    if (id == null) return;

    FollowListSheet.open(
      userId: id,
      initialType: type,
      showMyFollowing: user?.showMyFollowing,
      onMyFollowingCountChanged: (delta) {
        final isMe = id == SessionManager.instance.getUserID();
        if (!isMe) return;
        userData.update((val) {
          if (val == null) return;
          final next =
              ((val.followingCount ?? 0) + delta).clamp(0, 1 << 30);
          val.followingCount = next;
        });
        ...
        Get.bottomSheet(
                StoryViewSheet(
                  ...
                  onUpdateDeleteStory: (story) {
                    userData.update((val) =>
                        (val?.stories ?? []).removeWhere((element) => element.id == story?.id));
                  },
                ),
                isScrollControlled: true,
